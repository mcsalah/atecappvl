...
class oLocking
{
	var $lockTableName = "appviol2022_locking";
	var $ConfirmTime=250;
	var $UnlockTime=300;
	var $ConfirmAdmin;
	var $ConfirmUser;
	...
	function __construct()
	{
		...
		$this->ConfirmAdmin = "إحباط جلسة العمل الخاصة بك تحرير مسؤول٪ S";
		$this->ConfirmUser = "دورة تحرير الخاص بك مهلة";
		$this->LockAdmin = "تم تحرير محضر من قبل٪ S٪ S خلال دقائق";
		$this->LockUser = "تم تحرير السجل بواسطة مستخدم آخر";
		...
		if( $data = $qResult->fetchAssoc() )
		{
			...
			if($links){
				$str.='<a class="unlock" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'no\');return false;">'."فتح سجل".'</a>';
				$str.='<a class="edit" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'yes\');return false;">'."تعديل السجل".'</a>';
			}
			return $str;
		}
