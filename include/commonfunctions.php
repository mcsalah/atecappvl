...
function checkTableName($shortTName, $type=false)
{
	...
	if ("victime" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("enfants" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("respviolence" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tviolence" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tconsequences" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("t_dossier" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ecoute_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Victime");
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Enfants");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Respviolence");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("tviolence");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("tconsequences");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("t_dossier");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Ecoute_users");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function getMountNames()
{
	...
	$mounts[8] = "آب";
	$mounts[9] = "أيلول";
	$mounts[10] = "تشرين الأول";
	$mounts[11] = "تشرين الثانى";
	$mounts[12] = "كانُون الأَوَّل";
...
function verifyRecaptchaResponse( $response ) {
	...
	$errors = array();
	$errors["missing-input-response"] = "رمز الأمان غير صالحة.";
	$errors["invalid-input-response"] = "رمز الأمان غير صالحة.";
