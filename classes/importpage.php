...
class ImportPage extends RunnerPage
{
	...
	protected function prepareAndSentPreviewData()
	{
		...
		if( $returnJSON != false )
			...
			echo "الملف الذي تحاول استيراد لا يمكن تحليل";
...
//		$importFields = $this->pSet->getImportFields();
	...
	protected function getBasicReportText( $totalRecords, $addedRecords, $updatedRecords, $isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		{
			$reportText .= "استيراد إلى"." ".$this->strOriginalTableName.$lineBreak.
				str_format_datetime( db2time( now() ) ) .$lineBreak.$lineBreak;
		}
		
		$reportText .= mysprintf("%s الصورة من البيانات%الصورة بنجاح معالجة .", array($boldBegin.$importedReords.$boldEnd, $boldBegin.$totalRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s البيانات المضافة", array($boldBegin.$addedRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s البيانات المحدثة", array($boldBegin.$updatedRecords.$boldEnd)) .$lineBreak;
			
		if( $notImportedRecords )
			$reportText.= mysprintf("%s البيانات المعالجة مع الاخطاء", array($boldBegin.$notImportedRecords.$boldEnd));
