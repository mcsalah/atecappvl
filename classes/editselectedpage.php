...
class EditSelectedPage extends EditPage
{
	...
	protected function prepareButtons()
	{
		...
		{
			$label = str_replace( "%n%", $this->nSelected, "تحديث السجلات %n%" );
	...
	{
		$message = str_replace( array("%succeed%", "%total%"), array( "<strong>".$this->nUpdated."</strong>", "<strong>".$this->nSelected."</strong>" ), "%succeed% من أصل %total% سجلات تجديد بنجاح.");
		$this->setMessage( $message );
		
		if( $this->nUpdated != $this->nSelected ) {
			$message = str_replace( "%failed%", "<strong>".($this->nSelected - $this->nUpdated)."</strong>" , "فشل %failed% السجلات.");
			$this->setMessage( $message );
		}
	}
