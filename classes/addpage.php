...
class AddPage extends RunnerPage
{
	...
	protected function recheckUserPermissions()
	{
		...
		{
			$this->setMessage( "انتهت صلاحية جلسة العمل الخاصة بك." 
				. "<a href='#' id='loginButtonContinue" . $this->id . "'>" . "تسجيل الدخول" . "</a>" 
				. "لحفظ البيانات ." );
		}
		else
		{
	...
	protected function setSuccessfulUpdateMessage()
	{
		...
		if( $this->mode == ADD_INLINE ) 
			$infoMessage = ""."السجل أُدخل"."";
		else
			$infoMessage = "<strong><<< "."السجل أُدخل"." >>></strong>";
		...
		else 
		{
			...
			if( $this->editAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "edit", $keylink )."'>"."تحرير"."</a>&nbsp;";
				
			if( $this->viewAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "view", $keylink )."'>"."عرض"."</a>&nbsp;";
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != ADD_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."لم تتم إضافة التسجيلة"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
		{
			$this->message = "لم تتم إضافة التسجيلة".". ".$message;
		}
