...
*/
	...
	function checkCaptcha()
	{
		...
		if ( $captchaSettings["type"] == FLASH_CAPTCHA && @strtolower($this->captchaValue) != strtolower(@$_SESSION["captcha_" . $this->getCaptchaId()]) )
		{
			...
			$this->message = "رمز الأمان غير صالحة.";
		}
	...
	function getCaptchaHtml($_captchaFieldName)
	{
		...
		$typeCodeMessage = "اكتب الكود الذي تراه أعلاه";
		$path = GetCaptchaPath();
	...
	function noRecordsMessage()
	{
		...
		if( $this->pSetSearch->noRecordsOnFirstPage() && !$isSearchRun )
			return "لا شيء للمشاهدة. تشغيل بعض البحث.";

		if( !$this->rowsFound && !$isSearchRun )
			return "لا توجد بيانات حتى الآن .";

		if( $isSearchRun && !$this->rowsFound )
			return "لا يوجد أي سجلات ";
	}
	...
	function buildPagination()
	{
		...
		else
		{
			...
			if($this->maxPages > 1)
			{
				...
				if($counterstart != 1)
				{
					$pagination.= $this->getPaginationLink(1,"أول") . $advSeparator;
					$pagination.= $this->getPaginationLink($counterstart - 1,"سابق").$separator;
				}
				$pageLinks = "";
				...
				if($counterend != $this->maxPages)
				{
					$pagination.= $separator . $this->getPaginationLink($counterend + 1,"تال") . $advSeparator;
					$pagination.= $this->getPaginationLink($this->maxPages,"أخير");
				}
				if( $this->isBootstrap() )
	...
	function prepareRecordsIndicator($firstDisplayed, $lastDisplayed, $totalDisplayed)
	{
		...
		{
			$template = "عرض %first% - %last% من %total%";
	...
	*/
	...
	function getProceedLink()
	{
		...
		return '<span class="rnr-dbebrick">'
			...
			.  "انتقل إلى" . ' '. GetTableCaption( GoodFieldName( $this->tName ) )
			. '</a>'
			. "&nbsp;&nbsp;</span>";
	}
...
//		$this->AddJSFile('libs/js/migrationTool.js');
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."إضافة";
		if( $page == "edit" )
			return GetTableCaption($table).", "."تحرير"." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table)." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "تصدير";
		if( $page == "import" )
			return GetTableCaption($table).", "."استيراد";
		if( $page == "search" )
			return GetTableCaption($table)." - "."بحث متقدم";
		if( $page == "print" )
		...
		if( $page == "changepwd" )
			return "تغييرِ كلمة السر ";
		if( $page == "changepwd_success" )
			return "تغييرِ كلمة السر ";
		if( $page == "remind" )
			return "التذكير بكلمة المرور";
