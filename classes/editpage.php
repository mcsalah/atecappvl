...
class EditPage extends RunnerPage
{
	...
	public static function processEditPageSecurity( $table )
	{
		...
		if( $pageMode != EDIT_SIMPLE )
		{
			...
			if( !isLogged() || isLoggedAsGuest() )
				$messageLink = " <a href='#' id='loginButtonContinue'>". "تسجيل الدخول" . "</a>";
	...
	protected function setSuccessfulEditMessage()
	{
		...
		$this->setMessage( "<strong>&lt;&lt;&lt; "."جُدِد السجل". " &gt;&gt;&gt;</strong>" );
	}
	...
	protected function SecurityRedirect()
	{
		if($this->mode == EDIT_INLINE)
		{
			echo printJSON(array("success" => false, "message" => "السجل غير قابل للتحرير"));
			exit();
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != EDIT_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."لم تتم مراجعة التسجيلة"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
			$this->message = "لم تتم مراجعة التسجيلة".". ".$message;
