...
class LoginPage extends RunnerPage
{
	...
	protected function sendCodeBySMS()
	{
		...
		if( !$ret["success"] )
		{
			$this->message = "خطأ إرسال الرسالة"." ".$ret["error"];
		}
		else
		{
			$this->messageType = MESSAGE_INFO;
			$this->message = str_replace( "%phone%", $this->getMaskedPhone(), "تم إرسال رسالة نصية تحتوي على رمز ل: %phone%" );
		}
	...
	protected function reportSecondAuthStepResult( $verified )
	{
		...
		if( !$verified )
			$returnJSON["message"] = "رمز خاطئ";
		else
		{
			if( $this->mode == LOGIN_POPUP )
				$returnJSON["message"] = "لقد قمت بتسجيل الدخول بنجاح";
	...
	protected function refineMessage()
	{
		if( $this->message == "expired" )
			$this->message = "انتهت صلاحية جلسة العمل الخاصة بك." . " " . "يرجى تسجيل الدخول مرة أخرى .";
		elseif( $this->message == "invalidlogin" )
			$this->message = "تسجيل غير صحيح";
	...
	protected function isLoginAccessAllowed()
	{
		...
		$this->message = mysprintf( "رفض الوصول لدقائق٪ S", array($this->auditObj->LoginAccess()) );
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("Ecoute_users")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$strSQL = "select * from ".$this->connection->addTableWrappers( "Ecoute_users" )
			." where "
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "Ecoute_users", $data, $blobfields, $this );
			if( !$retval )
				return false;
