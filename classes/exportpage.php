...
class ExportPage extends RunnerPage
{
	...
	protected function getChooseFieldsCtrlMarkup()
	{
		...
		return '<select name="exportFields" multiple style="width: 100%;" data-placeholder="'."إِختار من فضلك".'" id="exportFields'. $this->id .'">'. implode( "", $options ) .'</select>';
	}
		...
		while( (!$nPageSize || $i < $nPageSize) && $row )
		{
			...
			foreach( $this->selectedFields as $field )
			{
				...
				if( IsBinaryType( $fType ) )
					$values[ $field ] = "بيانات Binary طويلة، لا يمكن عرضها ";
				else
