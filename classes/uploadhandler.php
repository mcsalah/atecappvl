...
class UploadHandler
{
    ...
    public function validate($uploadedFile, &$file, $error, $file_size, $index, $uploadDir) {
        ...
        if (!$file["name"]) {
            $file["error"] = "لم يتم توفير اسم الملف";
            return false;
        }
        if (!preg_match($this->options['accept_file_types'], $file["name"])) {
            $file["error"] = "نوع الملف غير مقبول";
            return false;
        }
        if ($this->options['max_file_size'] && (
            ...
            $file["error"] = mysprintf("حجم الملف يتجاوز حد كيلوبايت٪ S", array($this->options['max_file_size']));
            return false;
        }
        if ($this->options['min_file_size'] &&
            $file_size < $this->options['min_file_size'] * 1024) {
            $file["error"] = mysprintf("يجب أن لا يكون حجم الملف أقل من٪ S ك بايت", array($this->options['min_file_size']));
            return false;
        }
   		if (is_int($this->options['max_totalFile_size']) && (
                $this->getUploadFilesSize() + $file["size"] > $this->options['max_totalFile_size'] * 1024)
            ) {
            $file["error"] = mysprintf("إجمالي الملفات حجم يتجاوز حد كيلوبايت٪ S", array($this->options['max_totalFile_size']));
            return false;
        }
        if (is_int($this->options['max_number_of_files']) && (
            	...
            	if($this->options['max_number_of_files'] > 1)
            		$file["error"] = mysprintf("يمكنك تحميل أي ملفات أكثر من٪ S", array($this->options['max_number_of_files']));
            	else 
            		$file["error"] = "يمكنك تحميل ملف واحد فقط";
            return false;
        }
    ...
    public function handle_file_upload($uploadedFile, $index) {
		...
		if($this->pSet->isMakeDirectoryNeeded($this->field))
		{
			...
			{
				$file["error"] = "لا وجود تحميل المجلد";
				return $file;
			}
		}
